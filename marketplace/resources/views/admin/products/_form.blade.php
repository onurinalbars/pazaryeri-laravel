<div class="grid gap-4 md:grid-cols-2">
    <label class="text-sm font-semibold text-gray-700">
        Mağaza
        <select name="shop_id" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2">
            @foreach($shops as $id => $name)
                <option value="{{ $id }}" @selected(old('shop_id', $product->shop_id ?? '') == $id)>{{ $name }}</option>
            @endforeach
        </select>
    </label>
    <label class="text-sm font-semibold text-gray-700">
